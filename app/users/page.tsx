...
export default function UsersPage() {
  requireRole('admin')
  const users = db().prepare(`SELECT id, email, name, role, partner_id, active, created_at, last_login FROM users ORDER BY created_at DESC`).all() as any[]
  ...
  return (
    <div className="space-y-6">
      ...
      <UsersClient initial={users} labels={ROLE_LABELS as any} partners={partners} />
    </div>
  )
}
